...
function verificarGarantia() {
  ...
  if (alterado) {
    statusBox.className = 'status-box invalid';
    statusBox.innerHTML = `<h3>🚫 ALERTA: Componentes Alterados</h3><p style="font-size: 0.9em;">${discrepancias.join('<br>')}</p>`;
  } else {
    statusBox.className = 'status-box valid';
    statusBox.innerHTML = `<h3>✅ Garantía Válida</h3><p>Trazabilidad criptográfica comprobada.</p>`;
  }
}
